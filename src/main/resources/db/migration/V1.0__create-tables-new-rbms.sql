...
CREATE TABLE IF NOT EXISTS `fields` (
  ...
  `field_groups_id` bigint DEFAULT NULL,
  `is_multiple` bit DEFAULT NULL,
  `creation_date` TIMESTAMP DEFAULT NULL,
  `last_updated` TIMESTAMP DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE (`name`),
  INDEX (`field_groups_id`),
  CONSTRAINT fk_fields_field_groups FOREIGN KEY (`field_groups_id`) REFERENCES `field_groups` (`id`)
);
